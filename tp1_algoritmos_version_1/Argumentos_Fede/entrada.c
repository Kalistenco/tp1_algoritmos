...
/*considero que puedo ingresar menos ceros y se considerara como +0000 (es necesario el signo))*/
		for( var_aux = 1 ; cadena_palabra[var_aux] != '\0' ; var_aux++){
...
void imprimir_error_entrada( status_t_entrada estado_entrada ){
	...
	switch( estado_entrada ){
		case ST_ERROR_LECTURA_ARCHIVO:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA , MSJ_ERROR_LECTURA_ARCHIVO );
			break;
		case ST_ERROR_PALABRA_INCORRECTA:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA , MSJ_ERROR_PALABRA_INCORRECTA );
			break;
		case ST_ERROR_PALABRA_INCOMPLETA:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA , MSJ_ERROR_PALABRA_INCOMPLETA );
			break;
		case ST_PUNTERO_NULO:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA , MSJ_ERROR_PUNTERO_NULO );
			break;
		case ST_ERROR_ABRIR_ARCHIVO:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA , MSJ_ERROR_ABRIR_ARCHIVO );
			break;
		case ST_ERROR_NO_MEMORIA:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA , MSJ_ERROR_NO_MEMORIA );
			break;
		case ST_ERROR_CERRAR_ARCHIVO:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA , MSJ_ERROR_CERRAR_ARCHIVO );
			break;
		case ST_ERROR_LEER_CONSOLA:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA , MSJ_ERROR_LEER_CONSOLA );
			break;
		case ST_ERROR_PALABRA_FUERA_DE_RANGO:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA , MSJ_ERROR_PALABRA_FUERA_DE_RANGO);
		case ST_ERROR_VECTOR_VACIO:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA , MSJ_ERROR_VECTOR_VACIO);
			break;
		default:
			fprintf( stderr,"%s:%s\n", MSJ_ERROR_PREFIJO_ENTRADA,MSJ_ERROR_ENTRADA );
	}
	return;
}
